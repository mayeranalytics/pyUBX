...
#ifndef __PARSENMEAPAYLOAD_H__
#define __PARSENMEAPAYLOAD_H__
...
ParseNMEAPayload::parse(char buf[], size_t len)
{
    ...
    if(len == 0) { 
        #ifdef DEBUG
        printf("ParseNMEAPayload error: Zero length input\n");
        #endif
        onError(buf, len);
        return;
    }
    ...
    if(n_word == 0) {
        #ifdef DEBUG
        printf("ParseNMEAPayload error: No words\n");
        #endif
        onError(buf, len);
        return;
    }
    if(strcmp(words[0], "GPGGA")==0 || strcmp(words[0], "GNGGA")==0) {
        if(n_word != 15) {
            #ifdef DEBUG
            printf("ParseNMEAPayload error: %s message number of words is %lu, should be %d\n", words[0], n_word, 15);
            #endif
            return;
        }
        ...
        #ifdef DEBUG
        printf("ParseNMEAPayload error: Unknown NMEA message %s\n", words[0]);
        #endif
        onError(buf, len); 
    }
}
